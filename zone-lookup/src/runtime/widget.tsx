...
const Widget = (props: WidgetProps) => {
    ...
    // ---- Detect a phone-sized portrait viewport ----
    // visualViewport reflects the actual iframe viewport on iOS, including
    // orientation changes and the browser's dynamic toolbars.
    useEffect(() => {
        const update = () => {
            const viewport = window.visualViewport
            const width = viewport?.width ?? window.innerWidth
            const portrait = window.matchMedia('(orientation: portrait)').matches
            setIsPhonePortrait(width <= 600 && portrait)
        }
    ...
    const runLookupForPoint = useCallback(async (point: any, displayLabel?: string) => {
        // Dismiss the mobile keyboard before showing results so the portal gets the
    ...
    }, [config, ensureModules, clearGraphics, focusResults, pushRecent, inputId])
    ...
    /* Iframe mode: preserve the original desktop/landscape layout.
       The phone-portrait scroll workaround is scoped separately below. */
    &.zl-iframe-mode {
      height: 100%;
      min-height: 100%;
    }
    ...
    /* Phone portrait results are rendered through a React portal directly under
       document.body. This bypasses Experience Builder layout wrappers and any
       ancestor overflow rules before creating the only scrolling surface. */
    &.zl-mobile-result-portal {
    ...
    &.zl-mobile-result-portal .zl-result-card {
      box-sizing: border-box !important;
      width: 100% !important;
    ...
    &.zl-mobile-result-portal .zl-hero,
    &.zl-mobile-result-portal .zl-result-toolbar {
      flex: 0 0 auto;
    }
    ...
    &.zl-mobile-result-portal .zl-mobile-toolbar-spacer {
      flex: 1 1 auto;
    }

    &.zl-mobile-result-portal .zl-mobile-result-close {
      color: var(--primary, #0079c1);
      font-weight: 600;
    }

    &.zl-mobile-result-portal .zl-result-body {
    ...
    &.zl-mobile-result-portal:focus-visible {
      outline: 2px solid var(--primary, #0079c1);
      outline-offset: -3px;
    }
  ...
  `
    ...
    useEffect(() => {
        if (!mobileResultOverlay) return

        const focusTimer = window.setTimeout(() => resultsRef.current?.focus(), 0)
    ...
    }, [mobileResultOverlay, handleReset])

    const renderResultCard = (mobileOverlay: boolean) => (
        <div
            id={resultsId}
            ref={resultsRef}
            className={`zl-result-card${mobileOverlay ? ' zl-result-card-mobile-overlay' : ''}`}
            tabIndex={-1}
    ...
    return (
        ...
        >
          ...
          height stable in iframe embeds with no disappear/reappear flash. */}
            ...
            {/* Desktop and landscape keep the normal inline result card. */}
            {resultHtml && !mobileResultOverlay && renderResultCard(false)}
